...
{ STX, STX, STX, DLE, DLE, DLE, ETX, DLE, STX, DLE, ETX, ETX, ETX, STX, STX, DLE, DLE, DLE, STX, ETX };
...
int ut_CommandIO_SendResponse(void)
{
    ...
    if (!success) TraceBuffer(ZONE_INFO,
                              (uint8_t*)&randomData,
                              sizeof(randomData),
                              "%02x ",
                              "this random data killed the test:\n");
    CHECK(success);
...
int ut_CommandIO_CreateResponse_RTC(void)
{
    ...
    int i;
    for (i = 0; i < sizeof(g_RandomDataPool); i++) {
        g_RandomDataPool[i] = (uns8)rand() % 255;
    }
