...
struct __attribute__((__packed__)) cmd_set_fade {
    ...
    std::ostream& Write(std::ostream& out, size_t& indentation) const
    {
        ...
        return out << "0x" << std::hex << addrVal << " 0x" << std::hex << argbVal << ' ' << std::dec <<
               ntohs(fadeTmms) << ' ' << (int)parallelFade;
    }
...
//		if (offset > 0x7f) throw FatalError("Invalid Parameter, offset is greater than 127");
    ...
    std::ostream& Write(std::ostream& out, size_t& indentation) const
    {
        ...
        return out << "0x" << std::hex << argbVal_1 << " 0x" << std::hex << argbVal_2 << ' ' << std::dec << ntohs(
            fadeTmms) << ' ' << (int)(parallelAndOffset & 0x7F) << ' ' << (int)numberOfLeds << ' ' <<
               (int)(parallelAndOffset & 0x80);
    }
#endif
};
