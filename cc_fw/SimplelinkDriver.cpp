...
void SimplelinkDriver::setSecurityKey(SlHttpServerEvent_t const * const event) {
    memcpy(ProvisioningData.wlanSecurityKey.data(), event->EventData.httpPostData.token_value.data, event->EventData.httpPostData.token_value.len);
...
void SimplelinkDriver::SimpleLinkWlanEventHandler(SlWlanEvent_t *pSlWlanEvent) {
	...
	case SL_WLAN_CONNECT_EVENT:
        ...
        memcpy(Info.ConnectionSSID.data(), pSlWlanEvent->EventData.STAandP2PModeWlanConnected.ssid_name, pSlWlanEvent->EventData.STAandP2PModeWlanConnected.ssid_len);
...
void SimplelinkDriver::SimpleLinkHttpServerCallback(SlHttpServerEvent_t *pSlHttpServerEvent, SlHttpServerResponse_t *pSlHttpServerResponse) {
...
long SimplelinkDriver::addNewProfile(void) {
	if ((ProvisioningData.secParameters.Type == SL_SEC_TYPE_WPS_PBC) || (ProvisioningData.secParameters.Type == SL_SEC_TYPE_WPS_PIN)) {
		...
		sl_WlanConnect(ProvisioningData.wlanSSID.data(), strlen(ProvisioningData.wlanSSID.data()), 0, &ProvisioningData.secParameters, 0);
		return SimplelinkDriver::waitForConnectWithTimeout(CONNECT_TIMEOUT * 4);
    }
    else {
		long retRes = ERROR;
		retRes = sl_WlanProfileAdd(ProvisioningData.wlanSSID.data(), strlen(ProvisioningData.wlanSSID.data()), 0, &ProvisioningData.secParameters, 0, ProvisioningData.priority, 0);
		if (retRes < 0) {
			// Remove all profiles
