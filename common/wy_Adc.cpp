...
Adc::Adc(const enum channel& ch) :
		mChannel(ch) {
	if (mChannel == INVALID_CHANNEL)
		return;
...
uint16_t Adc::operator=(const Adc& rhs) const {
...
std::pair<uint16_t, uint32_t> Adc::get(void) const {
	...
	return std::make_pair((sample & 0x3fff) >> 2,
			(sample & 0xffffC000) >> 14);
}
