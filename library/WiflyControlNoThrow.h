...
/* --------------------- WLAN CONFIGURATION METHODES --------------------- */
     ...
     * @return Indexed by ::WiflyError
        ...
        <BR><B>NO_ERROR</B> is returned if no error occurred
     */
    uint32_t ConfModuleForWlan(const std::string& phrase,
                               const std::string& ssid,
                               const std::string& name = "Wifly_Light") const;
...
/* -------------------------- FIRMWARE METHODES -------------------------- */
     ...
     * @return Indexed by ::WiflyError
            ...
            <BR><B>NO_ERROR</B> is returned if no error occurred
     */
    uint32_t FwSetFade(const uint32_t argb,
                       const uint16_t fadeTime = 0,
                       const uint32_t addr = 0xffffffff,
                       const bool     parallelFade = false);
     ...
     * @return Indexed by ::WiflyError
       ...
       <BR><B>NO_ERROR</B> is returned if no error occurred
     */
    uint32_t FwSetGradient(const uint32_t argb_1,
                           const uint32_t argb_2,
                           const uint16_t fadeTime = 0,
                           const bool     parallelFade = false,
                           const uint8_t  length = NUM_OF_LED,
