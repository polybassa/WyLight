...
size_t ut_TelnetProxy_SendString(void)
{
    ...
    do {
        ...
        CHECK(testee.SendString(cmd, value));
        CHECK(
            cmdSetOptReplace.size() + hexReplacement.length() + CRLF.size() + cmd.size() + replacedValue.size() + CRLF.size() + cmdSetOptReplace.size() + hexReplacement.length() + CRLF.size() ==
