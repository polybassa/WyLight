...
struct FwCmdLoopOff : public FwCmdScript {
    ...
    std::ostream& Write(std::ostream& out, size_t& indentation) const override
    {
        return FwCmdScript::Write(out,
                                  --indentation) << TOKEN << ' ' << std::dec << (int)mReqFrame.data.loopEnd.numLoops;
    }
};
...
struct FwCmdSetColorDirect : public FwCmdSimple {
     ...
     */
    FwCmdSetColorDirect(uint32_t argb, uint32_t addr) : FwCmdSimple(SET_COLOR_DIRECT,
                                                                    sizeof(cmd_set_color_direct),
                                                                    false)
    {
     ...
     */
    FwCmdSetColorDirect(const uint8_t* pBuffer, size_t bufferLength) : FwCmdSimple(SET_COLOR_DIRECT,
                                                                                   sizeof(cmd_set_color_direct), false)
    {
...
struct FwCmdSetFade : public FwCmdScript {
    ...
    {
        return 0xff000000 | (uint32_t)mReqFrame.data.set_fade.red << 16 | (uint32_t)mReqFrame.data.set_fade.green << 8 |
               mReqFrame.data.set_fade.blue;
    }
...
struct FwCmdSetGradient : public FwCmdScript {
    ...
    FwCmdSetGradient(std::istream& is) : FwCmdScript(SET_GRADIENT, sizeof(cmd_set_gradient))
    {
        ...
        mReqFrame.data.set_gradient.Set(argb_1.argb(),
                                        argb_2.argb(), parallel, (uint8_t)offset, (uint8_t)length, fadeTime);
    }
     ...
     */
    FwCmdSetGradient(uint32_t argb_1,
                     uint32_t argb_2,
                     uint16_t fadeTime = 0,
                     bool     parallelFade = false,
                     uint8_t  length = NUM_OF_LED,
                     uint8_t  offset = 0) : FwCmdScript(SET_GRADIENT, sizeof(cmd_set_gradient))
    {
    ...
    {
        return FwCommand::argb(mReqFrame.data.set_gradient.red_1,
                               mReqFrame.data.set_gradient.green_1,
                               mReqFrame.data.set_gradient.blue_1);
    }
    ...
    {
        return FwCommand::argb(mReqFrame.data.set_gradient.red_2,
                               mReqFrame.data.set_gradient.green_2,
                               mReqFrame.data.set_gradient.blue_2);
    }
