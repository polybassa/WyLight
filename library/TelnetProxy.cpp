...
bool TelnetProxy::ExtractStringOfInterest(const std::string& buffer, const std::string& searchKey,
...
bool TelnetProxy::PerformWifiScan(std::string& result) const
{
    ...
    do {
        ...
        if (std::regex_search(answer.c_str(), startMatch,
                              std::regex(startRegEx)) &&
            std::regex_search(answer.c_str(), endMatch, std::regex(endRegEx)))
        {
